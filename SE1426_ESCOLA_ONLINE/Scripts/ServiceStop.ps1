...
Invoke-Command -Session $sess -ScriptBlock { param($service) if($service){ get-service "$service" | stop-service } } -ArgumentList($service)
...
{
	Write-Host Error trying to stop service: $_.Exception.Message
}
